...
- **Statistics** — days with each parent per month and per year, with percentages, plus a **fairness check**: weekend balance, recent tilts and streaks, computed on device from the assigned days.
- **WhatsApp heads-ups (voluntary)** — one tap opens WhatsApp with a prefilled message about the proposal you just sent (or the invitation). Each parent can optionally share their number in Settings so the button opens their chat directly; the app never sends anything by itself.
- **On-device AI (optional)** — on iPhones with Apple Intelligence (iOS 26+): ask for days in plain words ("I need the second weekend of August") and review the drafted days before sending; soften the tone of a proposal message; retell the month's numbers as a short narrative. Runs entirely on device — custody data never leaves the iPhone — and every feature degrades gracefully when unavailable. Building the AI features requires Xcode 26; with Xcode 16 the app compiles and works fully without them.
